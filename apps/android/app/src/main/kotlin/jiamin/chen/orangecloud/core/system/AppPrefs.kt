...
class AppPrefs @Inject constructor(
    ...
    val pinnedResources: Flow<Set<String>> = dataStore.data.map { it[KEY_PINNED_RESOURCES] ?: emptySet() }
    ...
    suspend fun togglePinnedResource(encoded: String) {
        dataStore.edit {
            val current = it[KEY_PINNED_RESOURCES] ?: emptySet()
            it[KEY_PINNED_RESOURCES] = if (encoded in current) current - encoded else current + encoded
        }
    }
    ...
    private companion object {
        ...
        val KEY_PINNED_RESOURCES = stringSetPreferencesKey("pref_pinned_resources")
    }
}
